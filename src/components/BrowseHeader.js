...
signOutfromAccount(navigate)
    ...
    return (<div className={`fixed ${topBar?"bg-neutral-800":navbarColor} w-full sm:px-12 px-0 py-2 flex-wrap  z-30 flex justify-between items-center transition-colors ease-in-out duration-300`}>
       ...
       <div className={`w-full sm:hidden flex bg-neutral-800 flex-col items-start relative   `}>
        <div onClick ={()=>{navigate('/browse')}} className="text-white text-lg sm:hidden pl-3 py-1 border-b w-full text-start">Home</div>
        <button className="text-white text-lg    sm:hidden border-b w-full pl-3 py-1 text-start" onClick={handleClick}>Signout</button>
        <button  className="text-white text-lg sm:hidden   w-full text-start pl-3 py-1 " onClick={()=>{toggle();settopBar(false)}} >Gpt-Search</button>
       </div>
       </div>
